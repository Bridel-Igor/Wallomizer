...
LRESULT SetSearchCollectionWindow::HandleMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		stFilt = new Static(Window(), "Filters:",		10,		40,		60,		20);
		btnRes = new Button(Window(), "Pick resolution",80,		40,		120,	20);
		btnAR = new Button(Window(), "Pick ratio",		210,	40,		120,	20);
		btnColor = new Button(Window(), "Pick color",	340,	40,		120,	20);

		stTag = new Static(Window(), "Tags:",			10,		70,		60,		20);
		edTag = new Edit(Window(), "",					80,		70,		380,	20);
		stTagInstruct[0] = new Static(Window(), "tagname - search fuzzily for a tag/keyword",				80,		100,	380,	15);
		stTagInstruct[1] = new Static(Window(), "- tagname - exclude a tag / keyword",						80,		115,	380,	15);
		stTagInstruct[2] = new Static(Window(), "+ tag1 + tag2 - must have tag1 and tag2",					80,		130,	380,	15);
		stTagInstruct[3] = new Static(Window(), "+ tag1 - tag2 - must have tag1 and NOT tag2",				80,		145,	380,	15);
		stTagInstruct[4] = new Static(Window(), "@username - user uploads",									80,		160,	380,	15);
		stTagInstruct[5] = new Static(Window(), "id : 123 - Exact tag search(can not be combined)",			80,		175,	380,	15);
		stTagInstruct[6] = new Static(Window(), "type : {png / jpg} - Search for file type(jpg = jpeg)",	80,		190,	380,	15);
		stTagInstruct[7] = new Static(Window(), "like : wallpaper ID - Find wallpapers with similar tags",	80,		205,	380,	15);

		btnCancel = new Button(Window(), "Cancel",		80,		230,	185,	20);
		btnOk = new Button(Window(), "Ok",				275,	230,	185,	20);
	...
	case WM_COMMAND:
	{
		...
		if COMMANDEVENT(btnColor)
		{
			ColorPickerWindow::windowThread(tmpColor, Window());
			return 0;
		}
